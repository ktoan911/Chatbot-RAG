...
from common.text import classification_query, extension_query, process_query
...
if prompt := st.chat_input("Bạn cần chúng tôi hỗ trợ gì?"):
    ...
    if is_needRAG:
        ...
        clean_query = vector_search.get_senmatic_search_result(reflected_query)
    else:
    ...
    if len(st.session_state.messages) > number_chat_history:
        st.session_state.messages = (
            st.session_state.messages[:1] + st.session_state.messages[3:]
        )
